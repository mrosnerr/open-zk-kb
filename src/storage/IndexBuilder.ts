...
function buildDataviewTable(source: string, kind: string, header: string): string[] {
  ...
  return [
    ...
    '  const editBtn = actions.createEl("a", { cls: "dv-action-btn", href: "obsidian://quickadd?choice=Edit%20Note&value-path=" + encodeURIComponent(p.file.path), attr: { "aria-label": "Edit", title: "Edit this note" } });',
    ...
    '    if (confirmed) { const row = delBtn.closest("tr"); if (row) row.remove(); await app.vault.trash(file, true); new Notice("Deleted: " + file.name); }',
    '  });',
