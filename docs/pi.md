...
When a Pi session starts, the extension derives the canonical project from the current working-directory basename and loads permanent preferences through the narrow `knowledge-context` preference-only transport. Automatic model context is the managed knowledge policy plus applicable retained preferences, not a compact project overview. It supplies that project and `client: "pi"` on every routine stored-knowledge call it initiates; caller-supplied values cannot widen the boundary. Before session initialization establishes a canonical project, it removes any model-provided project and still supplies `client: "pi"`, so project-required calls fail closed at server validation. The preference capsule enters model context through the system prompt, so it is available before the model responds and does not require a model-initiated `knowledge-search` call. It carries at most 12 preferences within an 800-token estimate (`ceil(UTF-16 code units / 4)`). Its contents are retained-memory guidance rather than authority for project requirements or status; an empty capsule means only that no applicable retained preference was found.
