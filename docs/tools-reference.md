...
- The `index` and `log` notes are auto-generated by the server on every project-scoped store, promote, archive, delete, or rebuild event (when `navigation.enableProjectIndex` / `navigation.enableProjectLog` are enabled).
