...
- **index** — a living catalog of all notes in the project, with wikilinks grouped by kind. Rebuilt on every store, archive, or promote event. One per project.
- **log** — an append-only chronological log of events (stores, promotions, archives). Each entry has a bold date prefix. One per project.

These notes are auto-generated by the server. Agents cannot create `index` or `log` notes manually via `knowledge-store`.
