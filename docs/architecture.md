...
open-zk-kb is shared, persistent memory for AI coding assistants, built on the Zettelkasten method and implemented as a Model Context Protocol (MCP) server. One knowledge base for all your tools — any MCP-compatible client (OpenCode, Claude Code, Cursor, Windsurf, Zed) can interact with the same atomic, linked notes.
