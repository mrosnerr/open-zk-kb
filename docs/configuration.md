...
Local usage counters and optional anonymous analytics sharing.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `telemetry.enabled` | boolean | `false` | Enable local SQLite counters and access timestamps (never leaves your machine) |
| `telemetry.share` | boolean | `false` | Send anonymous usage events to PostHog. Requires `enabled: true`. |
| `telemetry.id` | string | auto-generated | Anonymous UUID for PostHog `distinct_id`. Delete to reset identity. |
...
telemetry:
  enabled: true
  share: true     # anonymous analytics — set false to disable
  # id: <auto-generated UUID>
```

See [Telemetry](telemetry.md) for full details on what is collected, how to opt out, and how to verify.
