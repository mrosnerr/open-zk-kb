...
The checked-in and runtime defaults remain disabled:

```yaml
telemetry:
  enabled: false
  share: false
  # id: <auto-generated>  # Random approximate installation ID; delete to reset
```

Affirmative interactive consent changes both flags to `true` only after installation succeeds. `DO_NOT_TRACK=1` unconditionally prevents PostHog requests even when both flags are enabled. When local telemetry is disabled, open-zk-kb records no telemetry rows, skips note access tracking (`last_accessed_at` and `access_count`), and sends nothing externally.
