...
**Anonymous sharing** (`telemetry.share`) — When also enabled, anonymous session metadata (client, version, platform, vault size, tool usage counts) is sent to PostHog (EU Cloud) on the next server startup. No note content, search queries, or personal data is ever shared. See [Telemetry](telemetry.md) for the full event schema.
