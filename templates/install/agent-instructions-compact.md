...
- enduring preference/correction → personalization, with explicit universal/project/client scope. Apply the durability test: if it would stop mattering when the current project, configuration, subscription, or harness disappears, use decision/reference/domain instead. Weighed choice → decision; non-obvious gotcha → observation; looked up twice → reference; multi-step workflow discovered → procedure; useful URL → resource (`knowledge-ingest` first).
